...
################################################################################
    ...
    for ri in 2:p.num_layers-1
        index_diffu = p.layer_unknown * (ri-1) + 1 : p.layer_unknown * ri - 5
        for k in vcat(index_diffu,p.layer_unknown*ri-2)
            row = k
    ...
    for k in vcat(1:p.layer_unknown-5, p.layer_unknown-2)
        row = k
