...
function compute_row_col_val(rowind, colind, value, p, sol_0)
    ...
    for vi in 1:p.num_freq
        for ri in 1:p.num_layers
            for li in 1:p.n_rot
                ...
                ## SPT term
                s = put_row_col_val(rowind, colind, value, row, row, -p.ka[1], s)
            end
        end
    end
        ...
        for pi in 1:p.n_vib
            ...
            for pj in 1:p.n_vib
                if p.kVVmat[pi, pj] > 1e-8
                    ...
                    s = put_row_col_val(rowind, colind, value, row, row, -p.kVVmat[pi, pj], s)
                    s = put_row_col_val(rowind, colind, value, row, col, p.kVVmat[pj, pi], s)
                end
            end
        end
    end
    ...
    for k in 1:p.n_vib
        ...
        for pprime in 1:p.n_vib
            ...
            s = put_row_col_val(rowind, colind, value, row, col, val, s)
            s = put_row_col_val(rowind, colind, value, row, col-p.n_vib, val, s)
        end
    end
