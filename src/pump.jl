function alphaback(p, sol) # in m^-1
    tmp_factor = 8*π^3/3/p.h/p.c * 1e-36 * dipolematelesq(p) * p.f_pump * 1e-13 / p.df
...
function comppumprate!(p, alphab)
    ...
    alphaback = alphab[1]
    pavgforw = (1-exp(-alphaforw*p.L/100))/(alphaforw*p.L/100) * p.power / (1-triploss)
    pavgback = (1-exp(-alphaback*p.L/100))/(alphaback*p.L/100) * p.power / (1-triploss) *
                exp(-alphaforw*p.L/100) * R1
    ...
    for ri in 1:p.num_layers
        ...
        p.pump_IR[:, ri] =
        8*π^3/(3*p.h^2*p.c)*1e-36*dipolematelesq(p)/(π*p.radius^2)*1e-9 *
        (p.SHBF[:, ri]/p.df*pavgforw + p.SHBB[:, ri]/p.df*pavgback) / p.norm_time
     end
end
...
function dipolematelesq(p)
    matele = 0.
    if p.pumpbranch == "R"
        matele = p.dipolederivative^2*((p.JL+1)^2-p.K0^2)/(p.JL+1)/(2p.JL+1)
    elseif p.pumpbranch == "Q"
        matele = p.dipolederivative^2*p.K0^2/(p.JL+1)/p.JL
    else
        matele = p.dipolederivative^2*(p.JL^2-p.K0^2)/p.JL/(2p.JL+1)
    end
    return matele
end
